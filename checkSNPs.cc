...
/* Input: two SAM files and a .bed file
.bed file:
...
map<string, int> chromosome_names_1 = {
{"Lsat_1_v11_chr9", 9},
{"Lsat_1_v11_chr8", 8},
{"Lsat_1_v11_chr7", 7},
{"Lsat_1_v11_chr6", 6},
...
{"Lsat_1_v11_chr2", 2},
{"Lsat_1_v11_chr1", 1},
{"LSER_US96UC23_chr9", 9},
{"LSER_US96UC23_chr8", 8},
{"LSER_US96UC23_chr7", 7},
{"LSER_US96UC23_chr6", 6},
{"LSER_US96UC23_chr5", 5},
{"LSER_US96UC23_chr4", 4},
{"LSER_US96UC23_chr3", 3},
{"LSER_US96UC23_chr2", 2},
{"LSER_US96UC23_chr1", 1}
};

map<string, int> chromosome_names_2 = {
{"LSER_US96UC23_chr9", 9},
{"LSER_US96UC23_chr8", 8},
{"LSER_US96UC23_chr7", 7},
{"LSER_US96UC23_chr6", 6},
...
int main(int argc, char* argv[]) {
	...
	if(!snp_file) {
		cout << "SNP_file not good" << endl;
		exit(1);
	}
	cout << "2. Reading in SNP_file" << endl;
	while(snp_file) {
	...
	ifstream sam_file_1(argv[2]);
	ifstream sam_file_2(argv[3]);
	...
	while(sam_file_1 && sam_file_2) {
		...
		for(int s = 0; s < 1; s++) {
			...
			*sam_files[s] >> name;
			...
			while(name == prev_name && sam_file_1) {
			...
			*sam_files[s]>> MD; // de:
			...
			int chromosome_aligned = chromosome_names_1[chromosome];
			...
			} else if(mismatch + match < 2){
				alignment_outcomes[s] = 3;
				// total_Discarded++;
				//total_Recombinant++;
			} else if(mismatch_rate <= 0.0001) {
			...
			if(s == 0) { // changed temporarily to s == 0
				prev_name = name;
				if(alignment_outcomes[0] == 5 || alignment_outcomes[1] == 5) {
					total_Bad_Mismatch_Number++;
					OSS << " ******* Read is discarded, too many Mismatches" << endl << endl;
